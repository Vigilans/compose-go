...
func NewModelNamedMappingsResolver(opts *Options, configDetails types.ConfigDetails) interp.NamedMappingsResolver {
	...
	case "file":
		return scope.cachedMapping(name, key, func() (string, bool) {
			if value := fileObject["file"]; value != nil {
				return utils.Must(interpolateWithPath(scope.path.Next("file"), value, scope.opts)).(string), true
			}
			return "", false
		})
	case "environment":
