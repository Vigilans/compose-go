...
`
	...
	config, err := Load(configDetails, func(options *Options) {
		...
		options.NamedMappingsResolvers = []interp.NamedMappingsResolver{
			...
			NewModelNamedMappingsResolver(options, configDetails),
		}
	})
	assert.NilError(t, err)
		...
		Services: types.Services{
			"service-module": {
				...
				Extensions: types.Extensions{
					...
					"x-project-fields": fmt.Sprintf("%s %s", "test-project", filepath.Join(tmpdir, "module")),
			...
			"service-submodule": {
				...
				Extensions: types.Extensions{
					...
					"x-project-fields": fmt.Sprintf("%s %s", "test-project", filepath.Join(tmpdir, "module/submodule")),
...
secrets:
  access_key:
	...
	expected := &types.Project{
		...
		Configs: map[string]types.ConfigObjConfig{
			"user_from_file": {
				File: filepath.Join(tmpdir, "module1/testdata/file/user.txt"),
			},
			"user_from_env": {
				Environment: "USER",
				Content:     "jenny",
		...
		Secrets: map[string]types.SecretConfig{
			...
			"access_key": {
				File: filepath.Join(tmpdir, "module2/testdata/file/access_key.txt"),
				Extensions: types.Extensions{
